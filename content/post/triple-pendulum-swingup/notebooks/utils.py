def CartMultiPendulumSystem(m_cart=1, m1=1, l1=1, **kwargs):
    ...
    Args:
        ...
        m2, l2, ...
        encoder_ticks_per_rev: the number of ticks of the rotary encoder per revolution, adds no encoder if the value is (default) inf
    ...
    encoder_ticks_per_rev = kwargs.pop('encoder_ticks_per_rev', float('inf'))
    ...
    demux = builder.AddSystem(Demultiplexer(
        [1, num_pendulums, 1, num_pendulums]))
    builder.Connect(plant.GetOutputPort("cart_multi_pendulum_state"), demux.get_input_port(0))

    mux = builder.AddSystem(Multiplexer([1, num_pendulums]))
    if encoder_ticks_per_rev == float('inf'):
        builder.Connect(demux.get_output_port(0), mux.get_input_port(0))
        builder.Connect(demux.get_output_port(1), mux.get_input_port(1))
    else:
        builder.Connect(demux.get_output_port(0), mux.get_input_port(0))
        encoder = builder.AddSystem(RotaryEncoders([encoder_ticks_per_rev] * num_pendulums))
        builder.Connect(demux.get_output_port(1), encoder.get_input_port())
        builder.Connect(encoder.get_output_port(), mux.get_input_port(1))

    builder.ExportInput(plant.GetInputPort(
        "cart_multi_pendulum_actuation"), "f_cart")
    builder.ExportOutput(plant.GetOutputPort("cart_multi_pendulum_state"), "x")
    builder.ExportOutput(mux.get_output_port(), "y")
    builder.ExportOutput(scene_graph.get_query_output_port(), "query")

    cart_multi_pendulum = builder.Build()

    numerals = {1: 'Single', 2: 'Double', 3: 'Triple',
                4: 'Quadruple', 5: 'Quintuple', 6: 'Sextuple'}
    cart_multi_pendulum.set_name(
        f"Cart{numerals[num_pendulums]}Pendulum" if num_pendulums in numerals else f"Cart{num_pendulums}Pendulum")
...
def _CartMultiPendulumSdf(m_cart, **kwargs):
    ...
    cmap = LinearSegmentedColormap.from_list(
        "red_green_blue", [(1, 0, 0), (0, 1, 0), (0, 0, 1)])
    ...
    for i in range(1, num_pendulums+1):
        ...
        color = cmap((i-1) / (num_pendulums-1)
                     ) if num_pendulums > 1 else cmap(1)
